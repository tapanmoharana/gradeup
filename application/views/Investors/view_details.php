...
<div id="content-wrapper">
    ...
    <div class="page-header">			
        <div class="row">
            <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-dashboard page-header-icon"></i>&nbsp;&nbsp;List </h1>
            <div class="col-xs-12 col-sm-8">
                <div class="row">                    
                    <hr class="visible-xs no-grid-gutter-h">
                    <div class="pull-right col-xs-12 col-sm-auto"><a style="width: 100%;" class="btn btn-primary btn-labeled" href="<?=base_url($currentModule."/add")?>"><span class="btn-label icon fa fa-plus"></span>Add New</a></div>                        
                        ...
                        <div class="input-group no-margin">
                            <span style="border:none;background: #fff;background: rgba(0,0,0,.05);" class="input-group-addon"><i class="fa fa-search"></i></span>
                                ...
                                <?php
                                    ...
                                    {
                                ?>
                                <option value="<?=$slokas_details[$i]['project_name']?>"><?=$slokas_details[$i]['project_name']?></option>
                                <?php
                                    }
                                ?>
                            </select>
                ...
                <div class="panel-body">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody id="itemContainer">
                            ...
                            {
                            ?>
                            <tr <?php //=$slokas_details[$i]["status"]=="N"?"style='background-color:#FBEFF2'":""?>>
                                <td><?=$j?></td>
								   <td><?=$slokas_details[$i]['name']?></td>
                             <td><?php if($slokas_details[$i]['type']==1){echo "Company Overview"; } if($slokas_details[$i]['type']==2){echo "Press Releases"; } ?></td>
							   <td><img src="<?=base_url('uploads')?>/investors/<?=$slokas_details[$i]['thumb']?>" width="100px"></td>
							   <td><img src="<?=base_url('uploads')?>/investors/<?=$slokas_details[$i]['image']?>" width="100px" ></td>
							     <td><?=$slokas_details[$i]['description']?></td>
								
								 
                       
                              <td>
                                    <a href="<?=base_url($currentModule."/project_edit/".$slokas_details[$i]['id'])?>"><i class="fa fa-edit"></i></a>                                                                        
